...
pthread_mutex_t mutdist, mutvert;
...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<NT;i++)
    id[i] = i;
  pthread_mutex_init(&mutvert, 0);
  pthread_mutex_init(&mutdist, 0);
...
int isnewdist(long d[])
{
  ...
  pthread_mutex_lock(&mutdist);
  tree = root;
  for(;;){
    cv = compare(d,tree->dist);
    if(cv == 0){
      tree->count++;
      pthread_mutex_unlock(&mutdist);
      return 0;
    }
    if(cv < 0){
      if(tree->left){
...
int isnewvert(VERTEX *v)
{
  ...
  pthread_mutex_lock(&mutvert);
  tree = voot;
  for(;;){
    cv = compare2(v,tree->v);
    if(cv == 0){
      pthread_mutex_unlock(&mutvert);
      return 0;
    }
    if(cv < 0){
      if(tree->left){
