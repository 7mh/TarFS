...
void tweakset(int inset[]);
void untweakset(int inset[]);
void addset(int inset[]);
...
//          fprintf(logfd,"%3d%9d%9d eliminated\n",whoami,i,j);
    ...
    fprintf(logfd,"\n");
    fprintf(logfd,"%3d%6d%9d%9d%6d\n",whoami,REMOVE,best,utar,vtar);
    savegraph();
    REMOVE++;
    addset(inset);
  }
...
void savegraph()
{
  ...
  sprintf(filename,"r%02d.%03d",REMOVE,whoami);
  fd = fopen(filename, "w");
  for(i=0;i<N;i++){
    for(j=0;j<N;j++)
      fprintf(fd,"%1d",a[i][j]);
...
void makeset(int inset[])
{
  ...
  for(i=0;i<N;i++)
    inset[i] = 1;
  for(i=0;i<REMOVE;i++){
    do j = 169 + RND(180); while(!inset[j] || j == 199 || j == 349);
    inset[j] = 0;
  }
  for(i=j=0;i<350;i++){
    if(i == 199) utar = j;
...
void tweakset(int inset[])
{
  ...
  for(i=0;i<NTWEAK;i++){
    do j = 169 + RND(180); while(!inset[j] || j == 199 || j == 349);
    do k = 169 + RND(180); while(inset[k]);
    inset[j] = 0;
    inset[k] = 1;
    itweak[i] = k;
    otweak[i] = j;
...
void dfs(int dep)
{
  ...
  if(dep == N){
    found = 1;
    if(color[utar] != color[vtar]) bad = 1;
    return;
    if(!danfd){
